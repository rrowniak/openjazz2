...
const Animation = struct {
    ...
    fn init(allocator: std.mem.Allocator, animset: *const assets.Animset, 
        block_index: usize, anim_index: usize) !Animation {
        ...
        for (0..anim.frames.len) |i| {
            try anim.frames[i].sprite.set_palette(default_palette);
            ret.frame_sprites[i] = try anim.frames[i].sprite.to_sprite();
        }
        return ret;
    }

    fn draw(self: *Animation, x: usize, y: usize) void {
        self.frame_sprites[self.curr_fr_indx].draw(x, y);
    }

    fn next_frame(self: *Animation) void {
        self.curr_fr_indx += 1;
    ...
    fn get_wh(self: Animation) struct {w: usize, h: usize} {
...
pub const DiagAnimset = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, j2a_path: []const u8) !DiagAnimset {
        ...
        for (animset.blocks, 0..) |b, i| {
            for (0..b.anims.len) |j| {
                animations[cnt] = try .init(alloc, &animset, i, j);
                cnt += 1;
            }
        }
